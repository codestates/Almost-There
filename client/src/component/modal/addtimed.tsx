...
type AddTimefProps = {
  ...
  setMember: React.Dispatch<React.SetStateAction<Array<Member>>>,
  member: Array<Member>
}
let timeoutIdH: NodeJS.Timeout;
let timeoutIdM: NodeJS.Timeout;
let timeoutIdS: NodeJS.Timeout;
const AddTimed = ({ setTimeModal, member, setMember }: AddTimefProps) => {
  ...
  const clickComplete = () => {
    let hour0 = Number(hour) > 9 ? hour : `0${hour}`;
    let minute0 = Number(minute) > 9 ? minute : `0${minute}`;
    let second0 = Number(second) > 9 ? minute : `0${second}`;
    const time = `${hour0}:${minute0}:${second0}`
